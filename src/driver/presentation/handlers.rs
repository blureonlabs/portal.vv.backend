...
pub async fn list_drivers(
    ...
    let (drivers, total) = svc.list(limit, offset).await?;
    let data: Vec<DriverResponse> = drivers.into_iter().map(DriverResponse::from).collect();
...
pub async fn create_driver(
    ...
    let driver = svc.create(user.id, &role, body.profile_id, body.nationality, body.salary_type, body.room_rent_aed, body.commission_rate, body.joining_date, body.license_number).await?;
...
pub async fn update_driver(
    ...
    let body = body.into_inner();
    let role = user.role.clone();
    let driver = svc.update(user.id, &role, path.into_inner(), body.nationality, body.salary_type, body.room_rent_aed, body.commission_rate, body.joining_date, body.license_number).await?;
    Ok(HttpResponse::Ok().json(ApiResponse::ok(DriverResponse::from(driver))))
}
...
pub async fn create_driver_with_account(
    ...
    let driver: DriverResponse = svc.repo.create(user_id, &b.nationality, b.salary_type, b.room_rent_aed, b.commission_rate, b.joining_date, b.license_number.as_deref()).await?.into();
